...
class ReconciliationApp(tk.Tk):
    """Modern SaaS-style Account Reconciliation Tool."""

    # ── Design Tokens ─────────────────────────────────────
    BG          = "#f0f2f5"   # page background — light gray
    SURFACE     = "#ffffff"   # card/panel background — white
    SURFACE2    = "#f7f8fa"   # slightly off-white surface
    BORDER      = "#e2e8f0"   # subtle borders
    ACCENT      = "#3b82f6"   # blue primary
    ACCENT2     = "#7c3aed"   # violet secondary
    ACCENT3     = "#0891b2"   # cyan tertiary
    SUCCESS     = "#059669"   # green
    WARNING     = "#d97706"   # amber
    DANGER      = "#dc2626"   # red
    TEXT        = "#1e293b"   # primary text — dark
    TEXT2       = "#64748b"   # muted text
    TEXT3       = "#475569"   # hint text — darker for readability
    GREEN_BG    = "#dcfce7"
    BLUE_BG     = "#dbeafe"
    PURPLE_BG   = "#ede9fe"
    AMBER_BG    = "#fef3c7"
    ...
    def __init__(self):
        super().__init__()
        self.title("Reconciliation  ·  AccountSync")
        ...
        # Animation state
        self._pulse_phase   = 0
        self._btn_hover     = {}   # widget id → original bg
    ...
    def _build_ui(self):
        # ── HEADER BAR ────────────────────────────────────
        header = tk.Frame(self, bg=self.SURFACE, height=56)
        header.pack(fill="x")
        header.pack_propagate(False)

        # Animated accent stripe using Canvas
        self._header_canvas = tk.Canvas(
            header, width=4, height=56, bg=self.ACCENT,
            highlightthickness=0, bd=0)
        self._header_canvas.pack(side="left", fill="y")

        # Logo area
        logo_frame = tk.Frame(header, bg=self.SURFACE)
        logo_frame.pack(side="left", padx=(16, 0))

        tk.Label(logo_frame,
                 text="◈",
                 font=("Segoe UI", 20),
                 fg=self.ACCENT, bg=self.SURFACE).pack(side="left", pady=12)

        name_frame = tk.Frame(logo_frame, bg=self.SURFACE)
        ...
        # Thin separator
        tk.Frame(header, bg=self.BORDER, width=1).pack(side="left", fill="y", padx=20, pady=10)

        # Status pill — shows file / progress
        self._status_pill_frame = tk.Frame(header, bg=self.SURFACE2,
                                           highlightthickness=1,
                                           highlightbackground=self.BORDER)
        self._status_pill_frame.pack(side="left", pady=15, ipady=2, ipadx=12)
        self._status_dot = tk.Label(self._status_pill_frame, text="●",
                                    font=("Segoe UI", 8), fg=self.TEXT2,
                                    bg=self.SURFACE2)
        self._status_dot.pack(side="left", padx=(8, 4))
        self.lbl_file = tk.Label(self._status_pill_frame,
                                 text="No file loaded",
                                 font=("Segoe UI", 9), fg=self.TEXT3,
                                 bg=self.SURFACE2)
        self.lbl_file.pack(side="left", padx=(0, 8))

        # Right side — autosave badge
        right = tk.Frame(header, bg=self.SURFACE)
        right.pack(side="right", padx=16)
        self.lbl_autosave = tk.Label(right, text="",
                                     font=("Segoe UI", 8), fg=self.SUCCESS,
                                     bg=self.SURFACE)
        ...
        # ── ACTION TOOLBAR ────────────────────────────────
        toolbar = tk.Frame(self, bg=self.SURFACE2, pady=0)
        toolbar.pack(fill="x")

        inner = tk.Frame(toolbar, bg=self.SURFACE2)
        inner.pack(side="left", padx=12, pady=10)

        # Primary CTA buttons
        self._load_btn = self._make_pill_btn(
            inner, "  Load File", self.load_file,
            bg=self.ACCENT, fg="white", glow=True)
        self._load_btn.pack(side="left", padx=(0, 6))

        self._recon_btn = self._make_pill_btn(
            inner, "  Reconcile", self.run_recon,
            bg=self.ACCENT2, fg="white", glow=True)
        self._recon_btn.pack(side="left", padx=(0, 6))

        self._export_btn = self._make_pill_btn(
            inner, "  Export", self.export,
            bg="#f0fdf4", fg=self.SUCCESS, outline=self.SUCCESS)
        self._export_btn.pack(side="left", padx=(0, 14))

        # Thin separator
        tk.Frame(inner, bg=self.BORDER, width=1).pack(side="left", fill="y", pady=2)

        # Secondary row actions
        sec = tk.Frame(inner, bg=self.SURFACE2)
        sec.pack(side="left", padx=(10, 0))

        actions = [
            ("+ Above",        lambda: self.add_row("above"), self.SURFACE),
            ("+ Below",        lambda: self.add_row("below"), self.SURFACE),
            ("Edit",           self.edit_cell,                self.SURFACE),
            ("Delete Row",     self.delete_row,               "#fef2f2"),
            ("|",              None,                          None),  # separator
            ("Bulk Edit",      self.bulk_edit,                self.SURFACE),
            ("Show Match",     self.show_match,               self.SURFACE),
            ("+ Column",       self.add_column,               self.SURFACE),
            ("Hide/Show",      self.hide_show_columns,        self.SURFACE),
            ("Del Column",     self.delete_columns,           "#fef2f2"),
            ("Auto-Save",      self.set_auto_save_path,       self.SURFACE),
        ]
        for label, cmd, bg in actions:
            if label == "|":
                tk.Frame(sec, bg=self.BORDER, width=1).pack(side="left", fill="y", pady=2, padx=8)
                continue
            is_danger = bg == "#fef2f2"
            fg = self.DANGER if is_danger else self.TEXT3
            b = self._make_flat_btn(sec, label, cmd, bg=bg, fg=fg)
            b.pack(side="left", padx=2)

        # ── FILTER + STATUS STRIP ─────────────────────────
        fs = tk.Frame(self, bg=self.SURFACE, pady=0)
        fs.pack(fill="x")
        tk.Frame(self, bg=self.BORDER, height=1).pack(fill="x")

        fs_inner = tk.Frame(fs, bg=self.SURFACE)
        fs_inner.pack(fill="x", padx=14, pady=8)

        # "FILTER" label
        tk.Label(fs_inner, text="FILTER",
                 font=("Segoe UI", 7, "bold"),
                 fg=self.TEXT2, bg=self.SURFACE).grid(row=0, column=0,
                 rowspan=3, padx=(0, 12), sticky="ns")
        ...
        for i in range(3):
            rf = tk.Frame(fs_inner, bg=self.SURFACE)
            rf.grid(row=i, column=1, sticky="w", pady=1)

            tk.Label(rf, text=f"F{i+1}",
                     font=("Segoe UI", 7, "bold"),
                     fg=self.TEXT2, bg=self.SURFACE, width=2).pack(side="left", padx=(0, 6))

            cb = ttk.Combobox(rf, width=14, state="readonly", font=("Segoe UI", 9))
            cb.pack(side="left", padx=(0, 4))
            self.filter_cols.append(cb)

            ve = tk.Entry(rf, width=17, bg=self.SURFACE2, fg=self.TEXT,
                          insertbackground=self.ACCENT, font=("Segoe UI", 9),
                          relief="flat", highlightthickness=1,
                          highlightbackground=self.BORDER,
                          highlightcolor=self.ACCENT)
            ve.pack(side="left", padx=(0, 6))
            self.filter_vals.append(ve)
            ve.bind("<Return>", lambda e: self.apply_filter())

            bv = tk.BooleanVar(value=False)
            self.filter_blanks.append(bv)
            tk.Checkbutton(rf, text="Blanks", variable=bv,
        ...
        # Apply / Clear
        act_f = tk.Frame(fs_inner, bg=self.SURFACE)
        act_f.grid(row=0, column=2, rowspan=3, padx=12, sticky="ns")
        self._make_pill_btn(act_f, "Apply", self.apply_filter,
                            bg=self.ACCENT, fg="white").pack(pady=2, fill="x")
        self._make_flat_btn(act_f, "Clear", self.clear_filter).pack(pady=2, fill="x")

        # Status checkboxes
        st_f = tk.Frame(fs_inner, bg=self.SURFACE)
        st_f.grid(row=0, column=3, rowspan=3, padx=16, sticky="ns")
        tk.Label(st_f, text="STATUS", font=("Segoe UI", 7, "bold"),
                 fg=self.TEXT2, bg=self.SURFACE).pack(anchor="w", pady=(0, 2))
        self.status_filters = {}
        status_opts = [
            ("✅ Matched",           self.GREEN_FG),
            ("📊 Consecutive Match", self.BLUE_FG),
            ("🧮 Sum Match",         self.PURPLE_FG),
            ("🔄 Cross Matched",     self.AMBER_FG),
            ("❌ Unmatched",         self.DANGER),
        ]
        for lbl, col in status_opts:
            v = tk.BooleanVar(value=True)
            self.status_filters[lbl] = v
            tk.Checkbutton(st_f, text=lbl, variable=v,
                           command=self.apply_filter,
                           bg=self.SURFACE, fg=col,
                           selectcolor=self.SURFACE2,
                           activebackground=self.SURFACE,
        ...
        # Exclude
        ex_f = tk.Frame(fs_inner, bg=self.SURFACE)
        ex_f.grid(row=0, column=4, rowspan=3, padx=14, sticky="ns")
        tk.Label(ex_f, text="EXCLUDE", font=("Segoe UI", 7, "bold"),
                 fg=self.TEXT2, bg=self.SURFACE).pack(anchor="w", pady=(0, 2))
        self.exclude_fuel_var = tk.BooleanVar(value=False)
        tk.Checkbutton(ex_f, text="⛽ Fuel",
                       variable=self.exclude_fuel_var,
                       command=self.apply_filter,
                       bg=self.SURFACE, fg=self.WARNING,
                       selectcolor=self.SURFACE2,
                       activebackground=self.SURFACE, activeforeground=self.WARNING,
                       font=("Segoe UI", 8), cursor="hand2").pack(anchor="w")
        self.exclude_insurance_var = tk.BooleanVar(value=False)
        tk.Checkbutton(ex_f, text="🛡 Insurance",
                       variable=self.exclude_insurance_var,
                       command=self.apply_filter,
                       bg=self.SURFACE, fg=self.WARNING,
                       selectcolor=self.SURFACE2,
                       activebackground=self.SURFACE, activeforeground=self.WARNING,
        ...
        # ── STATS RIBBON ─────────────────────────────────
        ribbon = tk.Frame(self, bg=self.BG, pady=0)
        ribbon.pack(fill="x")
        self._stats_frame = ribbon

        self._stat_cards = {}
        labels = ["Rows", "Matched", "Consecutive", "Sum Match",
                  "Cross", "Unmatched", "Total Debit", "Total Credit"]
        colors = [self.TEXT3, self.GREEN_FG, self.BLUE_FG, self.PURPLE_FG,
                  self.AMBER_FG, self.DANGER, self.ACCENT3, self.ACCENT]

        for lbl, col in zip(labels, colors):
            card = tk.Frame(ribbon, bg=self.SURFACE,
                            highlightthickness=1,
                            highlightbackground=self.BORDER)
            card.pack(side="left", padx=(8, 0), pady=6, ipadx=10, ipady=4)
            tk.Label(card, text=lbl.upper(),
                     font=("Segoe UI", 6, "bold"),
                     fg=self.TEXT2, bg=self.SURFACE).pack(anchor="w", padx=8, pady=(4, 0))
            val_lbl = tk.Label(card, text="—",
                               font=("Segoe UI", 14, "bold"),
                               fg=col, bg=self.SURFACE)
            val_lbl.pack(anchor="w", padx=8, pady=(0, 4))
            self._stat_cards[lbl] = val_lbl

        # Keep old stats_bar for compatibility (hidden)
        self.stats_bar = tk.Label(self, text="", bg=self.BG, fg=self.TEXT2,
                                  font=("Segoe UI", 1))
        self.stats_bar.pack(fill="x")

        # ── TABLE AREA ────────────────────────────────────
        table_wrap = tk.Frame(self, bg=self.BG)
        table_wrap.pack(fill="both", expand=True, padx=8, pady=(0, 0))

        # Column headers via Treeview
        ...
        # ── PAGINATION BAR ────────────────────────────────
        pbar = tk.Frame(self, bg=self.SURFACE, pady=0)
        pbar.pack(fill="x", side="bottom")
        tk.Frame(pbar, bg=self.BORDER, height=1).pack(fill="x", side="top")

        pbar_inner = tk.Frame(pbar, bg=self.SURFACE)
        pbar_inner.pack(fill="x", padx=12, pady=8)

        # Nav buttons
        for sym, cmd in [("«", self.go_first), ("‹", self.go_prev),
                          ("›", self.go_next),  ("»", self.go_last)]:
            b = tk.Button(pbar_inner, text=sym, command=cmd,
                          bg=self.SURFACE2, fg=self.TEXT3,
                          activebackground=self.ACCENT,
                          activeforeground="white",
                          relief="flat", bd=0,
                          font=("Segoe UI", 12, "bold"),
            ...
            b.pack(side="left", padx=2)
            self._register_hover(b, self.SURFACE2, self.ACCENT)

        self.lbl_page = tk.Label(pbar_inner, text="Page — / —",
                                  fg=self.TEXT, bg=self.SURFACE,
                                  font=("Segoe UI", 9, "bold"))
        self.lbl_page.pack(side="left", padx=12)

        tk.Label(pbar_inner, text="Go to:", fg=self.TEXT2, bg=self.SURFACE,
                 font=("Segoe UI", 8)).pack(side="left", padx=(16, 4))
        self.jump_var = tk.Entry(pbar_inner, width=5,
                                  bg=self.SURFACE2, fg=self.TEXT,
        ...
        self._make_flat_btn(pbar_inner, "Go", self.go_to_page).pack(side="left", padx=4)

        self.lbl_rows = tk.Label(pbar_inner, text="",
                                  fg=self.ACCENT2, bg=self.SURFACE,
                                  font=("Segoe UI", 9, "bold"))
        self.lbl_rows.pack(side="right", padx=8)

    # ──────────────────────────────────────────────────────
    #  WIDGET FACTORIES
    # ──────────────────────────────────────────────────────

    def _make_pill_btn(self, parent, text, command,
                       bg=None, fg=None, outline=None, glow=False,
                       font=("Segoe UI", 9, "bold"), padx=14, pady=5):
        """Rounded pill-style button with hover animation."""
        bg  = bg  or self.SURFACE2
        fg  = fg  or self.TEXT
        btn = tk.Button(
        ...
        self._register_hover(btn, bg,
                              self._blend(bg, "#ffffff", 0.2))
        return btn

    def _make_flat_btn(self, parent, text, command,
                       bg=None, fg=None,
                       font=("Segoe UI", 8), padx=8, pady=4):
        """Flat ghost-style button."""
        bg = bg or self.SURFACE2
        fg = fg or self.TEXT3
        btn = tk.Button(
            parent, text=text, command=command,
            bg=bg, fg=fg,
            activebackground=self._blend(bg, self.ACCENT, 0.15),
            ...
            cursor="hand2")
        self._register_hover(btn, bg,
                              self._blend(bg, self.ACCENT, 0.18))
        return btn

    def _make_btn(self, parent, text, command, bg=None,
                  fg=None, font=("Segoe UI", 9, "bold"),
                  padx=12, pady=5):
        """Simple button for popups and dialogs."""
        bg = bg or self.ACCENT
        fg = fg or "white"
        btn = tk.Button(
            parent, text=text, command=command,
            bg=bg, fg=fg,
            activebackground=self._lighten(bg, 30),
            activeforeground=fg,
            relief="flat", bd=0,
            font=font,
            padx=padx, pady=pady,
            cursor="hand2")
        self._register_hover(btn, bg, self._lighten(bg, 20))
        return btn

    def _register_hover(self, widget, normal_bg, hover_bg):
        """Attach smooth hover color swap."""
        widget.bind("<Enter>", lambda e, w=widget, h=hover_bg: w.config(bg=h))
        widget.bind("<Leave>", lambda e, w=widget, n=normal_bg: w.config(bg=n))

    @staticmethod
    def _blend(hex1, hex2, t):
        """Linear interpolate between two hex colors. t=0→hex1, t=1→hex2."""
        try:
            def parse(h):
                h = h.lstrip("#")
    ...
    def _start_ambient_animation(self):
        """Subtle pulsing on the header accent stripe and status dot."""
        self._animate_tick()

    def _animate_tick(self):
        self._pulse_phase = (self._pulse_phase + 1) % 60
        # Pulse the header accent stripe color between ACCENT and ACCENT2
        t = (1 + __import__('math').sin(self._pulse_phase * 3.14159 / 30)) / 2
        blended = self._blend(self.ACCENT, self.ACCENT2, t * 0.6)
        try:
            self._header_canvas.config(bg=blended)
        except Exception:
            pass
        # Pulse the status dot when a file is loaded
        if self.df_original is not None:
            dot_t = (1 + __import__('math').sin(self._pulse_phase * 3.14159 / 15)) / 2
            dot_color = self._blend(self.SUCCESS, self.ACCENT3, dot_t * 0.7)
            try:
                self._status_dot.config(fg=dot_color)
            except Exception:
                pass
        self._anim_after_id = self.after(50, self._animate_tick)  # 20 fps
    ...
    def _apply_styles(self):
        ...
        style.configure("TCombobox",
                         ...
                         arrowcolor=self.TEXT2)
        style.map("TCombobox",
                  fieldbackground=[("readonly", self.SURFACE2)],
                  foreground=[("readonly", self.TEXT)])

    # ──────────────────────────────────────────────────────
    #  STATS RIBBON UPDATE (replaces old stats_bar.config)
    # ──────────────────────────────────────────────────────
    ...
    def _update_stats(self):
        ...
        if "_status" in df.columns:
            ...
            matched = cross_matched = unmatched = consec_matched = sum_matched = "—"
        ...
        card_vals = {
            "Rows":        f"{total:,}",
            "Matched":     f"{matched}" if isinstance(matched, str) else f"{matched:,}",
            "Consecutive": f"{consec_matched}" if isinstance(consec_matched, str) else f"{consec_matched:,}",
            "Sum Match":   f"{sum_matched}" if isinstance(sum_matched, str) else f"{sum_matched:,}",
            "Cross":       f"{cross_matched}" if isinstance(cross_matched, str) else f"{cross_matched:,}",
            "Unmatched":   f"{unmatched}" if isinstance(unmatched, str) else f"{unmatched:,}",
            "Total Debit":  f"₹{debit_sum:,.0f}",
            "Total Credit": f"₹{credit_sum:,.0f}",
        }
        for key, val in card_vals.items():
            if key in self._stat_cards:
                self._stat_cards[key].config(text=val)

        # Keep old stats_bar in sync (used by other code)
        self.stats_bar.config(
            text=(f"  Rows: {total:,}  |  Matched: {matched}  |  "
                  f"Consecutive: {consec_matched}  |  Sum: {sum_matched}  |  "
                  f"Cross: {cross_matched}  |  Unmatched: {unmatched}  |  "
    ...
    def _show_header_popup(self, path, preview_df):
        ...
        tree.tag_configure("normal",   background="#ffffff", foreground="#475569")
        tree.tag_configure("selected", background="#7c3aed", foreground="white")
        ...
        proceed_btn = tk.Button(
            ...
            bg="#059669", fg="white",
            activebackground=self._lighten("#059669"),
            activeforeground="white",
            relief="raised", bd=2,
        ...
        self._make_btn(btn_frame, "⚡ Use Row 0 (default)",
                       use_first, "#7c3aed").pack(side="left", padx=8)
        self._make_btn(btn_frame, "❌ Cancel",
                       popup.destroy, "#94a3b8").pack(side="left", padx=8)
    ...
    def _show_column_mapping(self, path):
        ...
        tk.Label(popup,
                 text="🗂  Column Mapping",
                 font=("Segoe UI", 14, "bold"), fg="#dc2626", bg="#ffffff"
        ...
        for i, (label, default, hints) in enumerate(mapping_fields):
            ...
            tk.Label(row_frame, text=label, fg="#475569", bg="#ffffff",
            ...
            prev_lbl = tk.Label(row_frame, text=f"  e.g. {preview}" if preview else "",
                                fg="#666", bg="#ffffff", font=("Segoe UI", 8))
        ...
        self._make_btn(btn_frame, "✅  Confirm Mapping", confirm_mapping, "#059669",
                       font=("Segoe UI", 11, "bold"), padx=18, pady=8
                       ).pack(side="left", padx=8)
        self._make_btn(btn_frame, "❌  Cancel", popup.destroy, "#94a3b8"
                       ).pack(side="left", padx=8)
    ...
    def _render_page(self):
        ...
        self.tree.tag_configure("matched",         background="#dcfce7", foreground="#16a34a")
        self.tree.tag_configure("consec_matched",  background="#dbeafe", foreground="#2563eb")
        self.tree.tag_configure("sum_matched",     background="#ede9fe", foreground="#7c3aed")
        self.tree.tag_configure("cross_matched",   background="#fef3c7", foreground="#d97706")
    ...
    # (Duplicate _update_stats removed — using stat-card version defined earlier)
    ...
    def show_match(self):
        ...
        tk.Label(popup,
                 text=f"🔗  Match ID: {mid}",
                 font=("Segoe UI", 13, "bold"), fg="#dc2626", bg="#ffffff"
                 ).pack(padx=16, pady=(12, 2), anchor="w")

        if match_note:
            tk.Label(popup,
                     text=f"📋  {match_note}",
                     font=("Segoe UI", 10), fg="#16a34a", bg="#ffffff"
        ...
        # ── FILTER BAR (triple) ──
        fbar = tk.Frame(popup, bg="#f1f5f9", pady=6)
        fbar.pack(fill="x", padx=0)

        tk.Label(fbar, text="  Filters:", fg="#1e293b", bg="#f1f5f9",
        ...
        for fi in range(3):
            row_frame = tk.Frame(fbar, bg="#f1f5f9")
            row_frame.grid(row=fi, column=1, sticky="w", pady=1, padx=4)

            tk.Label(row_frame, text=filter_labels[fi], fg="#475569", bg="#f1f5f9",
            ...
            val_entry = tk.Entry(row_frame, width=16, bg="#ffffff", fg="#1e293b",
                                 insertbackground="#1e293b", font=("Segoe UI", 8),
                                 relief="flat", highlightthickness=1,
                                 highlightbackground="#7c3aed", highlightcolor="#dc2626")
            val_entry.pack(side="left", padx=(0, 6))
            if main_val:
                val_entry.insert(0, main_val)
            popup_filter_vals.append(val_entry)

        btn_frame_f = tk.Frame(fbar, bg="#f1f5f9")
        btn_frame_f.grid(row=0, column=2, rowspan=3, padx=8, sticky="ns")

        # ── Table ──
        tbl_frame = tk.Frame(popup, bg="#ffffff")
        ...
        match_tree.tag_configure("debit",  background="#fef2f2", foreground="#dc2626")
        match_tree.tag_configure("credit", background="#dcfce7", foreground="#16a34a")

        # ── Summary ──
        summary_frame = tk.Frame(popup, bg="#f1f5f9")
        summary_frame.pack(fill="x", padx=10, pady=6)
        summary_lbl = tk.Label(summary_frame, text="",
                 font=("Segoe UI", 10, "bold"), fg="#475569", bg="#f1f5f9")
        summary_lbl.pack(side="left", padx=10, pady=6)

        self._make_btn(summary_frame, "❌ Close", popup.destroy, "#94a3b8",
        ...
        self._make_btn(btn_frame_f, "Apply",  apply_popup_filter, "#7c3aed",
                       font=("Segoe UI", 8, "bold"), padx=10, pady=4).pack(pady=2)
        self._make_btn(btn_frame_f, "Clear",  clear_popup_filter, "#94a3b8",
                       font=("Segoe UI", 8, "bold"), padx=10, pady=4).pack(pady=2)
    ...
    def add_column(self):
        ...
        # Column name
        name_frame = tk.Frame(popup, bg="#ffffff")
        name_frame.pack(fill="x", padx=20, pady=4)
        tk.Label(name_frame, text="Column Name:", fg="#475569", bg="#ffffff",
                 font=("Segoe UI", 10, "bold"), width=14, anchor="w"
                 ).pack(side="left")
        name_entry = tk.Entry(name_frame, width=30, bg="#f1f5f9", fg="#1e293b",
                              insertbackground="#1e293b", font=("Segoe UI", 10))
        name_entry.pack(side="left", padx=8)

        # Code editor
        expr_frame = tk.Frame(popup, bg="#ffffff")
        expr_frame.pack(fill="x", padx=20, pady=(10, 2))
        tk.Label(expr_frame, text="Python Code:", fg="#475569", bg="#ffffff",
                 font=("Segoe UI", 10, "bold"), anchor="w"
                 ).pack(anchor="w")

        expr_text = tk.Text(popup, height=6, bg="#f1f5f9", fg="#1e293b",
                            insertbackground="#1e293b", font=("Consolas", 10),
                            relief="flat", highlightthickness=1,
                            highlightbackground="#7c3aed", highlightcolor="#dc2626",
                            wrap="word", undo=True)
        ...
        tk.Label(popup, text=examples_text, fg="#666", bg="#ffffff",
        ...
        def do_preview():
            ...
            if not col_name:
                status_lbl.config(text="⚠  Please enter a column name.", fg=self.DANGER)
                return
            if not code:
                status_lbl.config(text="⚠  Please enter code.", fg=self.DANGER)
                return
            ...
            try:
                ...
                status_lbl.config(text=f"❌  Error: {e}", fg=self.DANGER)
        ...
        self._make_btn(btn_frame, "👁  Preview", do_preview, "#3b82f6",
                       padx=14, pady=6).pack(side="left", padx=6)
        self._make_btn(btn_frame, "✅  Apply", do_apply, "#059669",
                       padx=14, pady=6).pack(side="left", padx=6)
        self._make_btn(btn_frame, "❌  Cancel", popup.destroy, "#94a3b8",
                       padx=14, pady=6).pack(side="left", padx=6)

        status_lbl = tk.Label(popup, text="", fg="#475569", bg="#ffffff",
                               font=("Segoe UI", 9, "bold"))
        status_lbl.pack(side="bottom", padx=20, anchor="w")

        # ── Preview area (fills remaining space above buttons) ──
        preview_frame = tk.Frame(popup, bg="#ffffff")
    ...
    def _on_header_right_click(self, event):
        ...
        if self.hidden_columns:
            unhide_menu = tk.Menu(self._col_menu, tearoff=0, bg="#ffffff", fg="#475569",
    ...
    def hide_show_columns(self):
        ...
        # Quick actions frame
        quick_frame = tk.Frame(popup, bg="#ffffff")
        ...
        self._make_btn(quick_frame, "✅ Show All", select_all, "#059669",
                       font=("Segoe UI", 9, "bold"), padx=10, pady=3).pack(side="left", padx=4)
        self._make_btn(quick_frame, "⬜ Hide All", deselect_all, "#94a3b8",
                       font=("Segoe UI", 9, "bold"), padx=10, pady=3).pack(side="left", padx=4)
        ...
        for i, col in enumerate(all_cols):
            ...
            cb = tk.Checkbutton(
                ...
                bg=row_bg, fg="#475569", selectcolor="#f1f5f9",
                activebackground=row_bg, activeforeground="#dc2626",
            ...
            if col in self.hidden_columns:
                tk.Label(row_frame, text="HIDDEN", fg="#d97706", bg=row_bg,
        ...
        self._make_btn(btn_frame, "✅  Apply", apply_visibility, "#059669",
                       padx=14, pady=6).pack(side="left", padx=6)
        self._make_btn(btn_frame, "❌  Cancel",
                       lambda: (canvas.unbind_all("<MouseWheel>"), popup.destroy()),
                       "#94a3b8", padx=14, pady=6).pack(side="left", padx=6)
    ...
    def delete_columns(self):
        ...
        tk.Label(popup,
                 ...
                 font=("Segoe UI", 9), fg=self.DANGER, bg="#ffffff",
                 justify="left"
                 ).pack(padx=20, pady=(0, 10), anchor="w")
        ...
        for i, col in enumerate(all_cols):
            ...
            cb = tk.Checkbutton(
                row_frame, text=col, variable=var,
                bg=row_bg, fg="#475569", selectcolor="#fecaca",
                activebackground=row_bg, activeforeground="#dc2626",
                font=("Segoe UI", 10), cursor="hand2",
                anchor="w"
            )
        ...
        self._make_btn(btn_frame, "🗑  Delete Selected", do_delete, "#dc2626",
                       padx=14, pady=6).pack(side="left", padx=6)
        self._make_btn(btn_frame, "❌  Cancel",
                       lambda: (canvas.unbind_all("<MouseWheel>"), popup.destroy()),
                       "#94a3b8", padx=14, pady=6).pack(side="left", padx=6)
    ...
    def add_row(self, position="below"):
        ...
        tk.Label(
            ...
            font=("Segoe UI", 10, "bold"), fg="#dc2626", bg="#ffffff"
        ...
        for i, col in enumerate([c for c in cols if not c.startswith("_")]):
            tk.Label(frame, text=col, fg="#475569", bg="#ffffff",
                     font=("Segoe UI", 9)).grid(row=i, column=0, sticky="w", pady=2, padx=5)
            e = tk.Entry(frame, width=30, bg="#f1f5f9", fg="#1e293b",
                         insertbackground="#1e293b", font=("Segoe UI", 9))
        ...
        self._make_btn(win, "💾 Save Row", save, "#dc2626").pack(pady=10)
    ...
    def edit_cell(self):
        ...
        self._make_btn(win, "💾 Save Changes", save_edit, "#dc2626").pack(pady=10)
    ...
    def bulk_edit(self):
        ...
        tk.Label(popup, text="⚡ Bulk Edit — Update All Filtered Rows",
                 font=("Segoe UI", 11, "bold"), fg="#dc2626", bg="#ffffff"
        ...
        frm1 = tk.Frame(popup, bg="#ffffff")
        frm1.pack(fill="x", padx=20, pady=4)
        tk.Label(frm1, text="Select column:", fg="#475569", bg="#ffffff",
        ...
        frm2 = tk.Frame(popup, bg="#ffffff")
        frm2.pack(fill="x", padx=20, pady=4)
        tk.Label(frm2, text="Value to fill:", fg="#475569", bg="#ffffff",
                 font=("Segoe UI", 10), width=18, anchor="w").pack(side="left")
        val_entry = tk.Entry(frm2, width=26, bg="#f1f5f9", fg="#1e293b",
                             insertbackground="#1e293b", font=("Segoe UI", 10))
        val_entry.pack(side="left", padx=6)

        frm3 = tk.Frame(popup, bg="#ffffff")
        frm3.pack(fill="x", padx=20, pady=8)
        tk.Label(frm3, text="Apply to:", fg="#475569", bg="#ffffff",
                 font=("Segoe UI", 10), width=18, anchor="w").pack(side="left")
        scope_var = tk.StringVar(value="filtered")
        tk.Radiobutton(frm3, text=f"Filtered rows ({n_rows:,})",
                       variable=scope_var, value="filtered",
                       bg="#ffffff", fg="#1e293b", selectcolor="#7c3aed",
        ...
        preview_lbl = tk.Label(popup, text="", fg="#16a34a", bg="#ffffff",
        ...
        self._make_btn(btn_frame, "✅ Apply", apply_bulk, "#059669").pack(side="left", padx=8)
        self._make_btn(btn_frame, "❌ Cancel", popup.destroy, "#94a3b8").pack(side="left", padx=8)
