...
def _autosave_to_disk():
    """Save the current df_original to the auto-save path as a colour-coded Excel file.
    Called automatically after every edit (row add/delete/edit, bulk edit, etc.)."""
    path = st.session_state.get("autosave_path", "").strip()
    if not path or st.session_state.df_original is None:
        return
    try:
        import os
        os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
        df_exp = st.session_state.df_original[
...
def _dlg_show_match(mid):
    ...
    if mdf.empty:
        st.warning(f"No rows found for Match ID: {mid}")
        return
...
with st.sidebar:
    ...
    if st.button("✏️ Edit Cell", use_container_width=True, disabled=not _data_loaded):
        if _data_loaded:
            st.toast("Select a single row in the table, then click ✏️ Edit Row.", icon="ℹ️")
    ...
    if st.button(_show_match_label, use_container_width=True, disabled=(not _data_loaded or not _mid_for_sidebar)):
        _dlg_show_match(_mid_for_sidebar)
...
# ══════════════════════════════════════════════════════════
    ...
    if total > 0:
        ...
        # Always store the selected match ID so sidebar "Show Match" button can use it
        _selected_mid = ""
        if sel_rows and "_match_id" in df.columns:
            _first_real = start_idx + sel_rows[0]
            if _first_real < len(df):
                _selected_mid = str(df.iloc[_first_real].get("_match_id", "")).strip()
        st.session_state["_selected_mid"] = _selected_mid
